...
class CatalogMessages(db.Model):
    ...
    smsg_msg = db.relationship('SentMessages', lazy='select', back_populates='s_msg_msg')
    test_type = db.relationship('CatalogUserTestType', lazy='select', back_populates='msg_test_type')
...
class CatalogOperationsCenter(db.Model):
    ...
    phonenumber = db.Column(db.String(20), nullable=True)
    enabled = db.Column(db.Boolean, nullable=True, default=True)
    usr_op_cnt = db.relationship('User', lazy='select', back_populates='op_center')
...
class CatalogUserTestType(db.Model):
    ...
    name_short = db.Column(db.String(6), unique=True, nullable=True)
    enabled = db.Column(db.Boolean, nullable=True, default=True)
    usr_test_type = db.relationship('User', lazy='select', back_populates='test_type')
    msg_test_type = db.relationship('CatalogMessages', lazy='select', back_populates='test_type')
...
class SentMessages(db.Model):
    ...
    s_msg_usr = db.relationship('User', lazy='select', back_populates='smsg_usr')
    s_msg_msg = db.relationship('CatalogMessages', lazy='select', back_populates='smsg_msg')
